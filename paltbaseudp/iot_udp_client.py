...
import inspect
import ctypes
udp_server_socket = ('192.168.43.64',26711)
cycle_time=5
...
def udp_upload():
    ...
    while True:
        if cycle_time != 88888 :
            ...
            s.sendto(cpu_temp.encode(encoding="utf-8"),udp_server_socket)
...
def receve_cmd():
    ...
    while True:
        ...
        if recv_data[0:12] == 'change rate ' and recv_data[12:].isdigit():  
            ...
            s.sendto(send_data.encode(encoding="utf-8"), udp_server_socket)
        elif recv_data == "stop":
            cycle_time = 88888
            print("cycle_time is changed to be %s" % cycle_time)
            send_data="Successfully {} !!".format(recv_data)
            ...
            s.sendto(b'please input true cmd !!', udp_server_socket)
...
if __name__ == '__main__':
    ...
    upload_thread = threading.Thread(target = udp_upload, name = 'tcp_upload')
